...
// 陀螺仪初始化量程+-2000 度/秒于 1/(65536 / 4000) = 0.03051756*2
...
static double normAccz; /* z 轴上的加速度 */
/**
* @description: 根据 mpu 的 6 轴数据, 获取表征姿态的欧拉角
* @param {GyroAccel_Struct} *gyroAccel mpu 的 6 轴数据
* @param {EulerAngle_Struct} *EulerAngle 计算后得到的欧拉角
* @param {float} dt 采样周期 (单位 s)
* @return {*}
*/
    ...
    float dt)
{
    ...
    // 角速度融合加速度积分补偿值
    Gyro.x = gyroAccel->gyro.gyro_x * Gyro_Gr + KpDef * AccGravity.x +
    GyroIntegError.x; // 弧度制
    ...
    float yaw_G = gyroAccel->gyro.gyro_z * Gyro_G; // 将 Z 轴角速度陀螺仪值 转换为 Z 角度
    //秒 Gyro_G 陀螺仪初始化量程+-2000 度每秒于 1 / (65536 / 4000) = 0.03051756*2
    ...
    normAccz = gyroAccel->accel.accel_x * vecxZ + gyroAccel->accel.accel_y * vecyZ +
    gyroAccel->accel.accel_z * veczZ; /*Z 轴垂直方向上的加速度， 此值涵盖了倾斜时在 Z 轴角速度的
    向量和， 不是单纯重力感应得出的值*/
 } 
/*
**
* @description: 获取 Z 轴上的加速度 (如果已经倾斜,会考虑 z 轴上加速度的合成)
* @return {*}
*/
